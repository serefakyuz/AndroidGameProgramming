 ...
 *
 */
public class ContactListenerExample extends BaseGameActivity 
{
	...
	public Scene onLoadScene() 
	{
		...
		bodyOyuncu1.setLinearVelocity(-50, 25);
		bodyOyuncu2.setLinearVelocity(-50, -25);
		...
		this.physicsWorld.setContactListener(new ContactListener() 
		{
			...
			{
				// TODO Auto-generated method stub
				if(contact.getFixtureA().getBody() == physicsWorld.getPhysicsConnectorManager().findBodyByShape(spriteOyuncu1))
				{
					if(contact.getFixtureB().getBody() == physicsWorld.getPhysicsConnectorManager().findBodyByShape(spriteOyuncu2))
					{
						bodyOyuncu1.setLinearVelocity(5, 0);
					}
				}
				
				/*E�er 2 den fazla nesnemiz olsayd� o nesnelerin temas durumlar�n� da bu �ekilde yazabilirdik
				if(contact.getFixtureA().getBody() == physicsWorld.getPhysicsConnectorManager().findBodyByShape(spriteOyuncu3))
				{
					if(contact.getFixtureB().getBody() == physicsWorld.getPhysicsConnectorManager().findBodyByShape(spriteOyuncu4))
					{
						bodyOyuncu1.setLinearVelocity(5, 0);
					}
				}
